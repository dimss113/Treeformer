...
def gen_discrete_map(im_height, im_width, points):
    ...
    if not isinstance(points, np.ndarray):  # Convert points to numpy array
    ...
    points_np = points.round().astype(int)
...
class Crowd_TC(Base):
    def __init__(self, root_path, crop_size, downsample_ratio=8, method='train'):
        ...
        self.im_list = sorted(glob(os.path.join(self.root_path, 'images', '*.jpg')))
        self.xml_list = sorted(glob(os.path.join(self.root_path, 'labels', '*.xml')))

        print('Number of images [{}]: {}'.format(method, len(self.im_list)))
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(img_path).convert('RGB')
        objects = parse_pascal_voc_xml(xml_path)
        bboxes = [obj['bbox'] for obj in objects]  # Extract bounding boxes
        points = np.array([[bbox[0], bbox[1]] for bbox in bboxes])  # Extract points
        gauss_im = gen_discrete_map(img.size[1], img.size[0], points)  # Use points instead of bboxes
        gauss_im = torch.from_numpy(gauss_im).float()

        if self.method == 'train':
            return self.train_transform(img, points, gauss_im)  # Use points instead of bboxes
        elif self.method == 'val':
            wd, ht = img.size
            st_size = 1.0 * min(wd, ht)
            if st_size < self.c_size:
            ...
            return img, len(bboxes), name, gauss_im

    def train_transform(self, img, points, gauss_im):  # Adjusted to accept points instead of bboxes
        wd, ht = img.size
        st_size = 1.0 * min(wd, ht)
        # resize the image to fit the crop size
        if st_size < self.c_size:
        ...
        idx_mask = ((points[:, 0] >= j) & (points[:, 0] <= j + w) &
                    (points[:, 1] >= i) & (points[:, 1] <= i + h))
        points = points[idx_mask]

        gt_discrete = gen_discrete_map(h, w, points)
        down_w = w // self.d_ratio
        down_h = h // self.d_ratio
        gt_discrete = gt_discrete.reshape([down_h, self.d_ratio, down_w, self.d_ratio]).sum(axis=(1, 3))
        assert np.sum(gt_discrete) == len(points)
        ...
        return self.trans(img), gauss_im, torch.from_numpy(points.copy()).float(), torch.from_numpy(gt_discrete.copy()).float()
